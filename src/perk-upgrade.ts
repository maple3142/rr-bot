...
export default async (page: Page, strategy: PerkUpgradingStrategy) => {
	...
	if (duration) {
		...
		return parseTime(duration) * 1000
	}
	...
	const newDuration = (await getDurationString(page)) as string
	console.log(`PerkUpgrade: remaining ${newDuration}`)
	return parseTime(newDuration) * 1000
}
